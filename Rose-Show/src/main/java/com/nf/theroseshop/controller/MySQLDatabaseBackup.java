...
@Controller
@RequestMapping(value = "/mySQLDatabaseBackup")
public class MySQLDatabaseBackup {
    ...
    public List<Object> goToBackup(HttpServletRequest request){
        ...
        System.out.println(request.getSession().getServletContext().getRealPath("DataSql"));
        ...
        * */
        ...
        try {
            if (backup("127.0.0.1", "root", "adminMysql520.", basurl, bfday+".sql", "theroseshop")&&diaryService.insert(objectMap)>0) {
                objects.add("success");
                objects.add(basurl);
            } else {
                objects.add("error");
